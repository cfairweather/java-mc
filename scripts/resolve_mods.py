...
def parse_mods_txt():
    ...
    for lineno, raw in enumerate(MODS_TXT.read_text().splitlines(), 1):
        ...
        flags = set(parts[3:])
        if side not in SIDES or profile not in PROFILES or not flags <= {"datapack", "optional"}:
            sys.exit(f"{MODS_TXT}:{lineno}: bad side/profile/flag in '{raw}'")
        entries.append({"slug": slug, "side": side, "profile": profile,
                        "datapack": "datapack" in flags, "optional": "optional" in flags})
    return entries
...
def main():
    ...
    for w in wanted:
        ...
        queue.append((proj, w["side"], w["profile"], w["datapack"], None, w["optional"]))

    missing = []
    skipped = []
    while queue:
        proj, side, profile, is_datapack, dep_of, optional = queue.pop(0)
        pid = proj["id"]
        if pid in resolved:
            e = resolved[pid]
        ...
        if v is None:
            if optional:
                skipped.append(proj["slug"])
                print(f"  {proj['slug']:32} SKIPPED: no {loader} build for {game_version} (optional)", file=sys.stderr)
            else:
        ...
        for dep in v["dependencies"]:
            ...
            queue.append((dep_proj, side, profile, False, proj["slug"], False))
    ...
    lock = {
        ...
        "skipped_optional": sorted(skipped),
        "mods": entries,
    }
    outputs = {
    ...
    for profile in PROFILES:
        lines = [
            ...
            f"# Skipped (optional, no {game_version} build): {', '.join(sorted(skipped)) or 'none'}",
